...
function () {
  function Model(events, sync, attributes) {
    this.events = events;
    this.sync = sync;
    this.attributes = attributes; //get on() {return this.events.on}

    this.on = this.events.on; //get trigger() { return this.events.trigger}

    this.trigger = this.events.trigger; //get get() {return this.attributes.get}
  ...
  Model.prototype.save = function () {
    ...
    this.sync.save(this.attributes.getAll()).then(function (respone) {
      _this.trigger('save');
    }).catch(function () {
      _this.trigger('error');
    });
...
function () {
  ...
  Collection.prototype.fetch = function () {
    ...
    axios_1.default.get(this.rootUrl).then(function (respones) {
      respones.data.forEach(function (value) {
